...
static bool IsLight(Block block) {
	return
		...
		block == Block::LIGHT_GREEN ||
		block == Block::LIGHT_BLUE;
}
...
static uint32_t AmbientOcclusion(Block corner, Block side1, Block side2) {
	if (IsSolid(side1) && IsSolid(side2))
		return 3;
	else if (IsSolid(corner) && (IsSolid(side1) || IsSolid(side2)))
		return 2;
	else if (IsSolid(corner) || IsSolid(side1) || IsSolid(side2))
		return 1;
	else
...
Chunk* Chunk::RetrieveChunk(int x, int z) {
	if (x < 0 && z < 0)
		return m_Neighbors[static_cast<int>(Neighbors::NW)];
	else if (x >= XSIZE && z < 0)
		return m_Neighbors[static_cast<int>(Neighbors::NE)];
	else if (x < 0 && z >= ZSIZE)
		return m_Neighbors[static_cast<int>(Neighbors::SW)];
	else if (x >= XSIZE && z >= ZSIZE)
		return m_Neighbors[static_cast<int>(Neighbors::SE)];
	else if (x < 0)
		return m_Neighbors[static_cast<int>(Neighbors::W)];
	else if (x >= XSIZE)
		return m_Neighbors[static_cast<int>(Neighbors::E)];
	else if (z < 0)
		return m_Neighbors[static_cast<int>(Neighbors::N)];
	else if (z >= ZSIZE)
		return m_Neighbors[static_cast<int>(Neighbors::S)];
	else
		return this;
}

std::array<uint32_t, 4> Chunk::CalculateAO(int i, int j, int k,
	const std::array<std::array<int, 3>, 4>& offsets, uint8_t side) {
	std::array<Block, 4> corner = {};
	std::array<Block, 4> side1 = {};
	std::array<Block, 4> side2 = {};
	assertm(side >= 0 && side <= 2, "Wrong index.");
	for (int n = 0; n < offsets.size(); n++) {
		Chunk* chunk = RetrieveChunk(i + offsets[n][0], k + offsets[n][2]);
		corner[n] = chunk->m_Chunk(mod(i + offsets[n][0], XSIZE), j + offsets[n][1], mod(k + offsets[n][2], ZSIZE));
		switch (side)
		{
		case 0:
		{
			chunk = RetrieveChunk(i + offsets[n][0], k);
			side1[n] = chunk->m_Chunk(mod(i + offsets[n][0], XSIZE), j + offsets[n][1], k);
			chunk = RetrieveChunk(i + offsets[n][0], k + offsets[n][2]);
			side2[n] = chunk->m_Chunk(mod(i + offsets[n][0], XSIZE), j, mod(k + offsets[n][2], ZSIZE));
		}
		break;
		case 1:
		{
			chunk = RetrieveChunk(i + offsets[n][0], k);
			side1[n] = chunk->m_Chunk(mod(i + offsets[n][0], XSIZE), j + offsets[n][1], k);
			chunk = RetrieveChunk(i, k + offsets[n][2]);
			side2[n] = chunk->m_Chunk(i, j + offsets[n][1], mod(k + offsets[n][2], ZSIZE));
		}
		break;
		case 2:
		{
			chunk = RetrieveChunk(i + offsets[n][0], k + offsets[n][2]);
			side1[n] = chunk->m_Chunk(mod(i + offsets[n][0], XSIZE), j, mod(k + offsets[n][2], ZSIZE));
			chunk = RetrieveChunk(i, k + offsets[n][2]);
			side2[n] = chunk->m_Chunk(i, j + offsets[n][1], mod(k + offsets[n][2], ZSIZE));
		}
		break;
	...
	std::array<uint32_t, 4> aos = {};
	for (int n = 0; n < 4; n++) {
		aos[n] = AmbientOcclusion(corner[n], side1[n], side2[n]);
	}
	return aos;
}
	...
	uint32_t lightVal, Face face)
{
	...
	if (!IsLight(m_Chunk(position.x, position.y, position.z))) {
		...
		{
		case Face::Up:
			ambientOcclusionVals = CalculateAO(position.x, position.y, position.z,
	...
	// anysotropy fix
	if (ambientOcclusionVals[0] + ambientOcclusionVals[2] > ambientOcclusionVals[1] + ambientOcclusionVals[3]) {
		// flip the quad
		for (int i = 1; i <= vertices; i++) {
			uint32_t v = normalIndex << 29 | (position[0] + offsetx[mod(i, 4)]) << 24
				| (position[1] + offsety[mod(i, 4)]) << 15 | (position[2] + offsetz[mod(i, 4)]) << 10
				| textureCoords[mod(i, 4)];

			lightVal = (lightVal & 0xFFF0FFFF) | ambientOcclusionVals[mod(i, 4)] << 16;
		...
		for (int i = 0; i < vertices; i++) {
			...
			lightVal = (lightVal & 0xFFF0FFFF) | ambientOcclusionVals[i] << 16;
			target.emplace_back(v, lightVal);
		}
	}
}
...
{
	static constexpr int NORTH = static_cast<int>(Neighbors::N);
	static constexpr int EAST = static_cast<int>(Neighbors::E);
	static constexpr int WEST = static_cast<int>(Neighbors::W);
	static constexpr int SOUTH = static_cast<int>(Neighbors::S);
	if (j > 0 && ((m_Chunk(i, j - 1, k) == voidBlocks) || ...)) {
		...
		CreateQuad(target, { i, j, k }, textureCoords.bottom, // D
			{ 0, 0, 1, 1 }, glm::uvec4(0), { 1, 0, 0, 1 }, 3, lightVal, Face::Down);
	}
	if (j < YSIZE - 1 && ((m_Chunk(i, j + 1, k) == voidBlocks) || ...)) {
		...
		CreateQuad(target, { i, j, k }, textureCoords.top, // U
			{ 0, 1, 1, 0 }, glm::uvec4(1), { 1, 1, 0, 0 }, 2, lightVal, Face::Up);
	}
	if ((k == 0 && CheckNeighbor(m_Neighbors[NORTH], { i, j, ZSIZE - 1 }, voidBlocks...))
		...
		CreateQuad(target, { i, j, k }, textureCoords.side, // B
			{ 1, 0, 0, 1 }, { 0, 0, 1, 1 }, glm::uvec4(0), 5, lightVal, Face::Back);
	}
	if ((k == ZSIZE - 1 && CheckNeighbor(m_Neighbors[SOUTH], { i, j, 0 }, voidBlocks...))
		...
		CreateQuad(target, { i, j, k }, textureCoords.side, // F
			{ 0, 1, 1, 0 }, { 0, 0, 1, 1 }, glm::uvec4(1), 4, lightVal, Face::Front);
	}
	if ((i == 0 && CheckNeighbor(m_Neighbors[WEST], { XSIZE - 1, j, k }, voidBlocks...))
		...
		CreateQuad(target, { i, j, k }, textureCoords.side, // L
			glm::uvec4(0), { 0, 0, 1, 1 }, { 0, 1, 1, 0 }, 1, lightVal, Face::Left);
	}
	if ((i == XSIZE - 1 && CheckNeighbor(m_Neighbors[EAST], { 0, j, k }, voidBlocks...))
		...
		CreateQuad(target, { i, j, k }, textureCoords.side, // R
			glm::uvec4(1), { 0, 0, 1, 1 }, { 1, 0, 0, 1 }, 0, lightVal, Face::Right);
	}
}
	...
	int i, int j, int k, std::vector<Vertex>& target, const std::array<uint32_t, 4>& textureCoords)
{
	...
	for (int n = 0; n < 8; n++) {
		uint32_t v = (upNormal << 29) | (i + offsetx[n]) << 24 | (j + offsety[n]) << 15
			| (k + offsetz[n]) << 10 | textureCoords[mod(n, vertices)];
		uint32_t l = m_LightMap(i, j, k);
...
bool Chunk::FindNeighbors()
{
	...
	for (int i = -1; i <= 1; i += 2) {
		for (int k = -1; k <= 1; k += 2) {
			if (const auto it = m_ChunkMap->find({ m_Coords.x + i, m_Coords.z + k });
				it != m_ChunkMap->end()) {
				m_Neighbors[n++] = &it->second;
			}
			else { // border chunk, don't generate the mesh
...
{
	m_LightMap(i, j, k) = (m_LightMap(i, j, k) & 0xFFFF0FFF) | (val & 0xF) << 12;
}
...
{
	m_LightMap(i, j, k) = (m_LightMap(i, j, k) & 0xFFFFF0FF) | (val & 0xF) << 8;
}
...
{
	m_LightMap(i, j, k) = (m_LightMap(i, j, k) & 0xFFFFFF0F) | (val & 0xF) << 4;
}
...
{
	m_LightMap(i, j, k) = (m_LightMap(i, j, k) & 0xFFFFFFF0) | (val & 0xF);
}

uint8_t Chunk::GetBlueLight(int i, int j, int k) { return m_LightMap(i, j, k) & 0xF; }

std::array<Chunk*, 8> Chunk::GetNeighbors() const { return m_Neighbors; }
