...
namespace physics {
	...
	static constexpr float GRAVITY = 37.0f;
	...
	template<typename T>
		requires ChunkInfo<T>
	bool IsOnGround(Player& player, const T& chunkInfo);
};


template<typename T> requires ChunkInfo<T>
inline void physics::UpdatePlayer(Player& player, const T& chunkInfo, float deltaTime) {
...
template<typename T>
	requires ChunkInfo<T>
inline void physics::Spawn(Player& player, const T& chunkInfo) {
...
template<typename T> requires ChunkInfo<T>
inline bool physics::CalculateCollision(const glm::vec3& playerSpeed, const T& chunkInfo, glm::vec3& currentPosition) {
		...
		for (int j = startY; j <= endY; j++) {
			for (int k = startZ; k <= endZ; k++) {
				if (chunkInfo.IsBlockSolid({ i, j, k })) {
					...
					//if (Intersect(playerBbox, blockBbox)) {
...
template<typename T> requires ChunkInfo<T>
inline bool physics::IsOnGround(Player& player, const T& chunkInfo) {

	glm::vec3 playerPos = player.GetPosition();
	std::array<float, 2> vals = { PLAYER_HALF_WIDTH, -PLAYER_HALF_WIDTH };

	int y = std::floor(playerPos.y - PLAYER_BOTTOM_HEIGHT - 2 * EPS);
	for (float valx : vals) {
		for (float valz : vals) {
			int x = std::floor(playerPos.x + valx);
			int z = std::floor(playerPos.z + valz);

			if (chunkInfo.IsBlockSolid({ x, y, z }))
				return true;
		}
	}
	return false;
...
template<typename T> requires ChunkInfo<T>
inline void physics::CollisionResolution(Player& player, const T& chunkInfo, float deltaTime) {
	...
	player.SetHeadBob(true);
	if (collidedx) {
		distance.x = 0;
		player.SetHeadBob(false);
	}
	if (collidedy) {
	...
	if (IsOnGround(player, chunkInfo))
		player.SetOnGroundCamera(true);
	else
		player.SetOnGroundCamera(false);
