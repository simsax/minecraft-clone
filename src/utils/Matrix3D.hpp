...
template<typename T, size_t X, size_t Y, size_t Z>
inline Matrix3D<T, X, Y, Z>::Matrix3D():  m_Log2XZ(MyLog2(X * Z)), m_Log2Z(MyLog2(Z)) {
...
//template<typename T, size_t X, size_t Y, size_t Z>
//inline Matrix3D<T, X, Y, Z>::Matrix3D(T val) : Matrix3D<T, X, Y, Z>() {
...
inline Matrix3D<T, X, Y, Z>::Matrix3D(Matrix3D &&other) noexcept
        : m_Log2Z(0), m_Log2XZ(0), m_Data(nullptr) {
    swap(*this, other);
}
