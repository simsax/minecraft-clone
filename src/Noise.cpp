...
namespace noise {
    ...
    float Perlin2D(int xCoord, int yCoord, const std::array<int, 2> &offsets) {
        if (!offsets.empty())
            return glm::perlin(glm::vec2(xCoord, yCoord));
        else
            return glm::perlin(glm::vec2(xCoord + offsets[0], yCoord + offsets[1]));
    }
}