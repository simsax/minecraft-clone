...
void Game::CheckRayCast()
{
    ...
    while (!voxelFound && distance < maxDistance) {
        ...
        if (currentVoxel.y >= 0 && currentVoxel.y < m_ChunkManager.GetChunkSize()[1]
            && targetChunk->GetMatrix()(targetLocalVoxel[0], targetLocalVoxel[1], targetLocalVoxel[2])
                != Block::EMPTY) {
            voxelFound = true;
        }
    }
    ...
    if (voxelFound) {
        if (KeyPressed[GLFW_MOUSE_BUTTON_LEFT]) {
            ...
            if (m_HoldingBlock == Block::BEDROCK) {
                ...
                if (targetLocalVoxel[0] == 1 || targetLocalVoxel[2] == 1 || targetLocalVoxel[0] == chunkSize
                    || targetLocalVoxel[2] == chunkSize) {
                    UpdateNeighbor(currentVoxel, chunkSize, targetLocalCoord, Block::EMPTY);
                }
            }
        } else if (KeyPressed[GLFW_MOUSE_BUTTON_RIGHT]
            && !physics::Intersect(physics::CreatePlayerAabb(*playerPos), physics::CreateBlockAabb(previousVoxel))) {
            previousChunk->SetMatrix(
                previousLocalVoxel[0], previousLocalVoxel[1], previousLocalVoxel[2], m_HoldingBlock);
            m_ChunkManager.UpdateChunk(previousChunkCoord);
            // check if the target is in the chunk border
            if (previousLocalVoxel[0] == 1 || previousLocalVoxel[2] == 1 || previousLocalVoxel[0] == chunkSize
                || previousLocalVoxel[2] == chunkSize) {
