...
{
	GLCall(glUniform1iv(GetUniformLocation(name), length, v));
}
...
GLint Shader::GetUniformLocation(const std::string& name)
{
	...
	GLCall(GLint location = glGetUniformLocation(m_RendererID, name.c_str())); // retrieve the location of the uniform variable present in the shader
...
unsigned int Shader::CreateShader(const std::string& vertexShader, const std::string& fragmentShader) {
	...
	GLCall(glAttachShader(program, vs)); // attach shader to program object
	GLCall(glAttachShader(program, fs));
	GLCall(glLinkProgram(program)); // links the program
	GLCall(glValidateProgram(program)); // does validation

	GLCall(glDeleteShader(vs)); // frees the memory associated with shader object (deletes intermediates obj file)
...
unsigned int Shader::CompileShader(unsigned int type, const std::string& source) {
	GLCall(unsigned int id = glCreateShader(type)); // creates a shader object that contains the source code
	const char* src = source.c_str(); // pointer to beginning of the string
	GLCall(glShaderSource(id, 1, &src, NULL)); // sets the source code in the shader
	GLCall(glCompileShader(id)); // compile the shader

	// error handling
	int result;
	GLCall(glGetShaderiv(id, GL_COMPILE_STATUS, &result)); // query the shader object to get the value of some parameter
